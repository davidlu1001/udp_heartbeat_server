...
parser.add_argument(
    '--vif', metavar='str', type=str, required=False,
    help="The network interfalce of the virtual machine")
...
while True:
    ...
    # Do something here if NO heartbeat detected

	# e.g. add xen vm bridge
	#print "brctl addif xenbr0", args.vif
	#os.popen('brctl addif xenbr0' + ' ' + args.vif)

	# e.g. kill some process, "proxy" for example
	#os.popen('ps aux | grep -E "proxy" | grep -vw $$ | grep -vwE "grep|vi|vim" | awk \'{print "kill -9 "$2}\' | sh')
